...
const TAXA_PADRAO = 0.09; // 9% a.m. (era 0.05)
...
let MULTA_ATRASO = 0.18; 
let JUROS_DIARIOS_ATRASO = 0.01; 
...
function gerarGruposParcelas(isModal = false) {
    ...
    if (isModal) {
        // MODAL: Todos os grupos (1-12, 13-24, 25-36) com expansão
        const grupos = [
        ...
        // TELA PRINCIPAL: Apenas parcelas 1-6 FIXAS, sem grupos
        const gridFixo = document.createElement('div');
        gridFixo.className = 'parcelas-grid-fixo';
        ...
        for(let meses = 1; meses <= 6; meses++) {
            ...
            button.innerHTML = `${meses}x de <span class="valor-parcela-display">${formatBRL(valorParcela)}</span>`;
            gridFixo.appendChild(button);
        }
        
        container.appendChild(gridFixo);

        // Botão Ver Mais FIXO
        const verMaisBtn = document.createElement('button');
        verMaisBtn.className = 'btn-ver-mais-fixo';
