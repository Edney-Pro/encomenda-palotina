...
function carregarConfiguracoes() {
    const temaSalvo = localStorage.getItem('pessoal_tema') || 'dark';
    definirTema(temaSalvo);
    
    taxaAtual = parseFloat(localStorage.getItem('pessoal_taxa')) / 100 || TAXA_PADRAO;
    multaAtraso = parseFloat(localStorage.getItem('pessoal_multa')) / 100 || MULTA_ATRASO_PADRAO;
    jurosDiarioAtraso = parseFloat(localStorage.getItem('pessoal_juros_diario')) / 100 || JUROS_DIARIO_ATRASO_PADRAO;
...
function definirTema(tema) {
    ...
    localStorage.setItem('pessoal_tema', tema);
...
function salvarConfig() {
    ...
    if (!isNaN(novaTaxa) && novaTaxa > 0) localStorage.setItem('pessoal_taxa', novaTaxa);
    if (!isNaN(novaMulta) && novaMulta >= 0) localStorage.setItem('pessoal_multa', novaMulta);
    if (!isNaN(novosJuros) && novosJuros >= 0) localStorage.setItem('pessoal_juros_diario', novosJuros);
